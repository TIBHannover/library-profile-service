...
public class DocumentAssignmentFinderTest {
  ...
  private User newUser(final String name, final String password, final String initials,
    ...
    user.setCategories(Arrays.asList(categories).stream().map(c -> newDdcCategory(c)).collect(
        Collectors.toList()));
    return user;
  }
  ...
  private DocumentMetadata newDummyDocument(String... deweyDecimalClassifications) {
    ...
    documentMetadata.setDeweyDecimalClassifications(new HashSet<String>(Arrays.asList(
